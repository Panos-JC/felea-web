...
import { MentorDocument, useSetBioMutation } from '../../generated/graphql';
...
export const Bio: React.FC<BioProps> = ({
  ...
  const handleSubmit = async () => {
    const response = await setBio({variables: {mentorId, bio: bioState!}, refetchQueries: [{query: MentorDocument, variables: {mentorId}}]})
    setDialogOpen(false)
  }
  ...
  return (
    <GeneralCard title="About">
      ...
      <IconButton onClick={() => setDialogOpen(true)} className={classes.editIcon} size="small" color="secondary">
        <Edit />
      </IconButton>
      ...
      <Dialog fullWidth={true} maxWidth = {'md'} open={dialogOpen} onClose={() => setDialogOpen(false)}>
